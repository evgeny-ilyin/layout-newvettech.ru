...
export function calendarInit() {
	...
	const doctorId = String(calendarEl.dataset.doctorId);

	if (!apiUrl || !doctorId) {
		return;
	}

	const STORAGE_KEY_DOCTOR = `appointment_doctor_${doctorId}`;
	...
	if (appointmentModal) {
		...
		const observer = new MutationObserver(() => {
			...
			// Не вызывать, если шаг не первый
			if (typeof doctorData.step === 'number' && doctorData.step > 0) {
				return;
			}

			// Вызываем только при переходе из неактивного в активное состояние
	...
	if (btnSaveSlot) {
		btnSaveSlot.addEventListener('click', () => {
			...
			setSelectedDateTimeAsText(selectedDate, selectedTime);
			showStep(1);
		});
	}
	...
	// ==========================================
	// Показ выбранного времени и даты на втором и на успешном шаге
	// ==========================================

	function setSelectedDateTimeAsText(date, time) {
		if (!date || !time) {
			return;
		}

		selectedSlotSpan.textContent = `${formatDate(date)}, ${time}`;
		successStepDate.textContent = `${formatDate(date)}`;
		successStepTime.textContent = `${time}`;
	}
	...
	function getDoctorData() {
		try {
			return JSON.parse(localStorage.getItem(STORAGE_KEY_DOCTOR)) || {};
		} catch {
			return {};
		}
	}
	...
	function saveDoctorData(data = {}) {
		...
		localStorage.setItem(STORAGE_KEY_DOCTOR, JSON.stringify(result));
	...
	function restoreStep() {
		...
		if (typeof doctorData.step === 'number' && doctorData.step >= 0 && doctorData.step <= 2) {
			...
			setSelectedDateTimeAsText(doctorData.date, doctorData.time);
			return;
		}
		showStep(0);
	}
